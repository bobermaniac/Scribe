    ...
    def validate_classes(classes)
      self.all_properties_should_satisfy(classes) do |a_class, a_prop|
        ...
        unless a_prop.reference_type? and a_prop.readonly?
          STDERR.puts "\tProperty `#{a_prop.name}` of class `#{a_class.name}` marked as nonnull but this contract could be satisfied only if property marked as readonly. Generated classes have no default property initializers and property will be set to default (nil) value after designated initializer was called. You should mark property as readonly or remove nonnull type hint."
          next false
        end
        next true
      end
  ...
  class ShouldUseOnlyImmutableTypesForPropertiesValidator < Validator
    def validate_classes(classes)
      self.all_properties_should_satisfy(classes) do |a_class, a_prop|
        ...
        STDERR.puts "\tProperty `#{a_prop.name}` of class `#{a_class.name}` have type #{a_prop.type.qualified_string} which is not guaranteed to be immutable. Using of mutable types breaks immutable class contract. You have to avoid using this type. If you sure that this type is immutable use `hint immutable` to declare it in explicit way"
        next false
      end
    end
  end
  ...
  class ClassCouldNotBeSerializableWithBlockPropertiesValidator < Validator
    def validate_classes(classes)
      self.all_properties_should_satisfy(classes) do |a_class, a_prop|
        ...
        STDERR.puts "\tClass `#{a_class.name}` requires to be persistent via NSCoding protocol implementation but it have property `#{a_prop.name}` which have block type. Block types can't be serialized via NSKeyedArchiver. You should remove 'archivable' annotation or avoid using block property types for serializable class."
        next false
      end
    end
  end
